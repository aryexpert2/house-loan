...
export default function CreateListing() {
  ...
  const handleImageSubmit = (e)=> { 
    if (files.length > 0 && files.length + formData.imageUrls.length < 7) {
      ...
      setImageUploadError('Yon can only upload 6 images per listing');
      setUploading(false);
    }
  }
      ...
      <form className='flex sm:flex-row flex-col justify-center rounded-md gap-4'>
        ...
        <div className='flex flex-col w-full gap-4'>
          ...
          <div className="flex gap-4">
            <input className='p-3 border border-gray-300 rounded w-full' onChange={(e)=>setFiles(e.target.files)} type="file" id='images' accept='image/*' multiple='true' />
          ...
          <p className='text-red-700 text-sm'>{imageUploadError && imageUploadError}</p>
          {
            formData.imageUrls.length > 0 && formData.imageUrls.map((url, index)=>(
                <div key={url} className="flex items-center border justify-between p-3">  
                  <img src={url} alt="listing image" className="w-20 h-20 object-containrounded-lg" />,
                  <button onClick={()=>handleRemoveImage(index)} type='button' className='p-3 text-red-700 rounded-lg uppercasr hover:opacity-75'>Delete</button>
                </div>
            ))
          }
